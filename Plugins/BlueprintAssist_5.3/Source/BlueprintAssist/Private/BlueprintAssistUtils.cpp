...
{
	static const TSet<FName> ObjTypes = { UEdGraphSchema_K2::PC_SoftObject, UEdGraphSchema_K2::PC_SoftClass, UEdGraphSchema_K2::PC_Class, UEdGraphSchema_K2::PC_Object }; 
...
FString FBAUtils::GetDefaultPinValue(UEdGraphPin* Pin)
{
	if (Pin)
	{
		...
		if (PC == UEdGraphSchema_K2::PC_Struct)
		{
			...
			{
				// for structs, we need to append the property name to each value
				// source =	V1, V2"
				// result = "(P1=V1, P2=V2)
				TArray<FString> Values;
				Pin->DefaultValue.ParseIntoArray(Values, TEXT(","));
				...
				for (TFieldIterator<BA_PROPERTY> PropertyIter(Struct); PropertyIter; ++PropertyIter)
				{
					...
					{
						Values[Index] = PropertyIter->GetName() + "=" + Values[Index];
					}
					else
					{
						return "";
				...
				StructValue.RemoveFromEnd(",");
				StructValue = FString::Printf(TEXT("(%s)"), *StructValue);
...
bool FBAUtils::TrySetDefaultPinValuesFromString(UEdGraphPin* Pin, const FString& NewDefault, FString* OutError)
{
	...
	{
		NewDefaultTextValue = FText::FromString(NewDefault);
	}
	if (PC == UEdGraphSchema_K2::PC_Struct)
	{
		if (const UStruct* Struct = Cast<UStruct>(Pin->PinType.PinSubCategoryObject))
