...
void FBAToolbar::MakeToolsSubMenu(FMenuBuilder& InMenuBuilder)
{
	InMenuBuilder.BeginSection("BlueprintAssistTools", INVTEXT("Tools"));
	{
		...
		// debug menu
		InMenuBuilder.AddMenuEntry(INVTEXT("Open Debug Menu"), INVTEXT("Opens a debug menu to see info about the current graph"), FSlateIcon(), FExecuteAction::CreateLambda([]()
		{
			FGlobalTabmanager::Get()->TryInvokeTab(FName("BADebugMenu"));
		}));

		// welcome screen
		InMenuBuilder.AddMenuEntry(INVTEXT("Open Welcome Screen"), INVTEXT("Opens the Blueprint Assist Welcome Screen"), FSlateIcon(), FExecuteAction::CreateLambda([]()
		{
			FGlobalTabmanager::Get()->TryInvokeTab(SBAWelcomeScreen::GetTabId());
		}));

		// setting changes window
		InMenuBuilder.AddMenuEntry(INVTEXT("Setting Changes"), INVTEXT("Opens a window which displays your local changes for Blueprint Assist settings"), FSlateIcon(), FExecuteAction::CreateLambda([]()
		{
